...
app.whenReady().then(() => {
  ...
  if (shot) {
    ...
    ipcMain.once('ui-ready', async () => {
      ...
      if (hair || beard || clothing || items || zoomBone || cam || facing || tab || ambient) { win.webContents.send('test-appearance', { hair, beard, clothing, items, cam, facing, tab, ambient, zoomHead: !!process.env.PZICON_ZOOMHEAD, zoomBone, zoomRadius: Number(process.env.PZICON_ZOOMR) || undefined }); await once('test-appearance-done'); }
