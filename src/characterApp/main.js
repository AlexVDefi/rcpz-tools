...
app.whenReady().then(() => {
  ...
  if (shot) {
    ...
    ipcMain.once('ui-ready', async () => {
      ...
      const cam = process.env.PZICON_CAM, facing = process.env.PZICON_FACING, tab = process.env.PZICON_TAB, ambient = process.env.PZICON_AMBIENT;
      const stop = !!process.env.PZICON_STOP;
      if (hair || beard || clothing || items || zoomBone || cam || facing || tab || ambient || stop) { win.webContents.send('test-appearance', { hair, beard, clothing, items, cam, facing, tab, ambient, stop, zoomHead: !!process.env.PZICON_ZOOMHEAD, zoomBone, zoomRadius: Number(process.env.PZICON_ZOOMR) || undefined }); await once('test-appearance-done'); }
