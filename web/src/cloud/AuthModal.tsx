...
// Email + password sign-in / sign-up dialog for the optional online sharing feature.
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000000aa', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <form onClick={(e) => e.stopPropagation()} onSubmit={submit}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          <div style={{ fontSize: 16, fontWeight: 700 }}>{mode === 'in' ? 'Sign in' : 'Create an account'}</div>
          <span role="button" onClick={onClose} title="close" style={{ cursor: 'pointer', color: 'var(--muted)', padding: '0 4px' }}>✕</span>
        </div>
        <div style={{ color: 'var(--muted)', fontSize: 12.5, marginBottom: 14 }}>Optional - only needed to upload and share renders online.</div>
        ...
        <div style={{ textAlign: 'center', marginTop: 12, fontSize: 12.5, color: 'var(--muted)' }}>
          {mode === 'in' ? (
            <>No account? <a role="button" onClick={() => { setMode('up'); setErr(''); setInfo(''); }} style={{ cursor: 'pointer' }}>Create one</a></>
          ) : (
            <>Already have one? <a role="button" onClick={() => { setMode('in'); setErr(''); setInfo(''); }} style={{ cursor: 'pointer' }}>Sign in</a></>
          )}
        </div>
      </form>
    </div>
